      ...
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        <defs>
          <linearGradient id="radarFill" x1="0%" y1="0%" x2="100%" y2="100%">
            ...
            <stop offset="100%" stopColor={colors.lavender} stopOpacity="0.15" />
          </linearGradient>
        </defs>
        {gridRings.map((r, ri) =>
          categories.map((_, i) => {
        ...
        {points.map((p, i) => (
          <circle key={i} cx={p.x} cy={p.y} r={3.5} fill={colors.gold} style={{ filter: `drop-shadow(0 0 4px ${colors.gold})` }} />
        ))}
      </svg>
      {points.map((p, i) => (
        <div
      ...
      <div style={{ height: 5, borderRadius: 3, background: "rgba(255,255,255,0.06)", overflow: "hidden" }}>
        <div
          style={{
            ...
            background: color,
            borderRadius: 3,
            boxShadow: `0 0 8px ${typeof color === 'string' && color.startsWith('#') ? color + '40' : 'rgba(201,168,76,0.3)'}`,
          }}
        />
      </div>
    </div>
    ...
    <div style={{ padding: "60px 16px 16px", fontFamily: fonts.sans, background: `linear-gradient(180deg, #0D0D14, #0A0A0F)`, height: "100%", overflow: "hidden" }}>
      ...
      <div
        style={{
          ...
          marginBottom: 14,
          textAlign: "center",
          boxShadow: `0 4px 24px rgba(0,0,0,0.3), 0 0 30px rgba(201,168,76,0.06)`,
        }}
      >
        <div style={{ fontSize: 9, color: colors.gold, letterSpacing: 2, textTransform: "uppercase", marginBottom: 8 }}>
  ...
  return (
    ...
    >
      ...
      <div style={{ maxWidth: 440, zIndex: 2 }}>
        ...
        >
          Measure what{" "}
          <span style={{ background: `linear-gradient(135deg, ${colors.lavender}, ${colors.lavenderLight}, ${colors.gold})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
            truly matters
          </span>
        </div>
