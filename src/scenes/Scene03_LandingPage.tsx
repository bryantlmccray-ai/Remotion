...
const PhoneScreen: React.FC = () => {
  ...
  return (
    <div style={{ width: "100%", height: "100%", background: `linear-gradient(180deg, #0D0D14, #0A0A0F)`, display: "flex", flexDirection: "column", padding: "60px 20px 20px", fontFamily: fonts.sans }}>
      <div style={{ transform: `translateY(${scrollY}px)` }}>
        <div style={{ textAlign: "center", marginBottom: 24 }}>
          <div style={{ fontSize: 24, fontFamily: fonts.serif, fontWeight: 700, background: `linear-gradient(135deg, ${colors.gold}, ${colors.goldLight}, ${colors.rose})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", lineHeight: 1.2, marginBottom: 8 }}>
            Transform Your Relationship
          </div>
          <div style={{ fontSize: 11, color: colors.textSecondary, lineHeight: 1.5 }}>Science-backed tools for deeper connection</div>
        </div>
        <div style={{ background: colors.gradientGold, borderRadius: 30, padding: "14px 24px", textAlign: "center", marginBottom: 20, boxShadow: `0 8px 32px rgba(201,168,76,0.4)` }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
          {[["50K+", "Couples"], ["4.9★", "Rating"], ["95%", "Success"]].map(([num, label]) => (
            <div key={label} style={{ flex: 1, textAlign: "center", padding: "14px 8px", borderRadius: 14, background: `linear-gradient(145deg, rgba(201,168,76,0.08), rgba(155,142,196,0.04))`, border: `1px solid ${colors.border}` }}>
              <div style={{ fontSize: 18, fontWeight: 700, background: colors.gradientGold, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>{num}</div>
        ...
        {[
          ...
          <div key={title} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", borderRadius: 14, background: `linear-gradient(135deg, rgba(201,168,76,0.06), rgba(155,142,196,0.03))`, border: `1px solid ${colors.border}`, marginBottom: 8 }}>
